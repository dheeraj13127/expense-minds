...
import { getStatisticsMonthlyURL } from "../../../../url/URL";
...
const StatisticsMonthly = () => {
  ...
  const fetchStatisticsMonthly = useCallback(async () => {
    const statisticsData = await axios.get(
      `${getStatisticsMonthlyURL}?month=${result.year()}-${result.format(
        "MM"
  ...
  const handleFetchNewStatisticsData = async () => {
    try {
      ...
      const recordsData = await axios.get(
        `${getStatisticsMonthlyURL}?month=${data}&amountType=${amountType}`,
        {
          headers: {
            Authorization: "Bearer " + token,
          },
